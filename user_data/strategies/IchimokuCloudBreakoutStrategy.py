...
class IchimokuCloudBreakoutStrategy(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Chikou Span: close shifted backward by displacement (so current chikou is
        # the close that will be plotted at position -displacement)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        # Long: price above cloud, Tenkan above Kijun, Chikou above old price
        ...
        chikou_bullish = dataframe["chikou"] > dataframe["close"].shift(int(self.kijun_period.value))

        long_setup = (
            above_cloud & tenkan_above_kijun & chikou_bullish & (dataframe["volume"] > 0)
        )

        # Short: price below cloud, Tenkan below Kijun, Chikou below old price
        ...
        chikou_bearish = dataframe["chikou"] < dataframe["close"].shift(int(self.kijun_period.value))
